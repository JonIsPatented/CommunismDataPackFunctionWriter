execute as @e[tag=Overseer,limit=1] store result score @s item_count run data get block -396 76 -65 Items[{id:"minecraft:glowstone"}].Count
scoreboard players set @e[tag=Overseer] multiplier 24
...
data remove block -396 76 -65 Items[{id:"minecraft:glowstone"}]